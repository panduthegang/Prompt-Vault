...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* ========================================== */}
      <aside
        className={`hidden lg:flex bg-vault-dark text-vault-cream rounded-[28px] p-4 sm:p-5 flex-col justify-between shrink-0 border-2 border-vault-dark shadow-sm transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] lg:sticky lg:top-6 lg:h-[calc(100vh-48px)] ${
          isCollapsed ? 'w-[84px]' : 'w-[280px] xl:w-[300px]'
        }`}
      >
              ...
              return (
                <div key={nav.id} className="relative group/tooltip flex items-center justify-center w-full">
                  <button
                    ...
                    } rounded-xl font-sans text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer ${
                      isActive
        ...
        <div
          className={`pt-5 border-t border-vault-cream/15 flex items-center ${
            isCollapsed ? 'justify-center relative group/user' : 'gap-3'
          }`}
        >
          <img
